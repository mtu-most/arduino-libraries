...
uint8_t _lcd_width = 16, _lcd_height = 2;
...
void message(char const *line1, char const *line2)
{
	...
	lcd.setCursor(0, 0);
	for (unsigned i = 0; i < _lcd_width / 2 - strlen(line1) / 2; ++i)
		lcd.print(" ");
	lcd.print(line1);
	for (unsigned i = 0; i < _lcd_width / 2 - strlen(line1) / 2 + strlen(line1); ++i)
		lcd.print(" ");
	for (unsigned y = 1; y < _lcd_height / 2; ++y) {
		lcd.setCursor(0, y);
		for (unsigned i = 0; i < _lcd_width; ++i)
			lcd.print(" ");
	}
	lcd.setCursor(0, _lcd_height / 2);
	lcd.print(line2);
	for (unsigned i = 0; i < _lcd_width / 2 - strlen(line2) / 2 + strlen(line2); ++i)
		lcd.print(" ");
	for (unsigned y = _lcd_height / 2 + 1; y < _lcd_height; ++y) {
		lcd.setCursor(0, y);
		for (unsigned i = 0; i < _lcd_width; ++i)
			lcd.print(" ");
	}
